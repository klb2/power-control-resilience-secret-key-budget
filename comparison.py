...
from rayleigh import expectation_skg_rate_rayleigh, expectation_skg_rate_rayleigh_conditional_bob

from util import to_decibel, db_to_linear

def calculate_constant_power_level_balance(env_config):
    rv_bob = env_config.get('rv_bob') if "rv_bob" in env_config else float(env_config.get('snr_bob_db'))
    rv_eve = env_config.get('rv_eve') if "rv_eve" in env_config else float(env_config.get('snr_eve_db'))
    lam_bob = 1./db_to_linear(rv_bob) if isinstance(rv_bob, (float, int)) else 1./rv_bob.std()
    lam_eve = 1./db_to_linear(rv_eve) if isinstance(rv_eve, (float, int)) else 1./rv_eve.std()
    prob_tx = env_config['prob_tx']
    message_length = env_config['message_length']
    max_power_db = env_config['max_power_db']
    max_power = 10**(max_power_db/10.)
    ...
    sol = optimize.root_scalar(func_root_expect, args=(lam_bob, lam_eve, target_expect),
                               bracket=(0, max_power), x0=1)
    power = sol.root
...
def adaptive_power_conditional_expectation(state, env_config):
    ...
    min_budget = env_config['rv_alert_duration'].ppf(1-env_config['outage_prob_alert'])*env_config['message_length']
    ...
    if budget_buffer <= 0:
        ...
        def func_min_target(power, channel_bob, lam_eve, max_power, budget_buffer):
            ...
            _part1 = np.log(expectation_skg_rate_rayleigh_conditional_bob(power, channel_bob, lam_eve))
            _part2 = .002*budget_buffer*np.log(power)
            return -(_part1 - _part2)
        try:
            sol = optimize.minimize(func_min_target, args=(channel_bob, lam_eve, max_power, budget_buffer),
                                    bounds=((1, max_power),), x0=.5*max_power)
            #power_db = sol.root
            #print(sol)
            power = sol.x[0]
...
def calculate_min_budget(env_config, safety_margin=.2):
